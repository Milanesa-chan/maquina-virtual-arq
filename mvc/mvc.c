...
int traduce(FILE *arch, int muestra)   //ya tenemos la lista de rotulos creada
{
                        ...
                        if (*palabra == '[')   //
                        {
                            ...
                            if((dospuntos = strchr(palabra, ':')) != NULL)      //dospuntos= despues de dos puntos
                            {
                                ...
                                *(regTemp+2) = '\0';
                                //regTemp = DX
                                //12,          , 2 ,dx
                                errorbase=errorbase || registroBase(linea, memoria, arg, regTemp);//justificar
    ...
    if (errorconst)
        printf("\nError de constante, invalida.");
    if (errorbase)
        printf("\nError de base, no valida.");

    //registros[3] = 1000;        //registro ES=1000
    return (errorsin || errorrot || errorconst || errorbase);
}
...
{//segmentos 0 = DATA, 1 = EXTRA, 2 = STACK//por defecto en 500
    ...
    {
        registros[3]=segmentos[0]+registros[2];//ES=DATA+DS
        registros[5]=segmentos[1]+registros[3];//SS
    }
    else
    {
        registros[3]=-1;                        //ES
        registros[5]=registros[2]+segmentos[0]; //SS
    }
    registros[6]=segmentos[2];                  //SP
    registros[0]=registros[5]+registros[6];     //PS
    for(int i=0; i<16; i++)
    {
